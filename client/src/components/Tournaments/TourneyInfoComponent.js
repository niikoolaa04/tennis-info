...
import { findTournament, getPlayerFromName } from '../../utils/utils';
...
function TourneyInfoComponent() {
  ...
  const getWinner = async(name) => {
    await getPlayerFromName(name).then((data) => {
      setWinnerId(`${data}`);
    })
  }

  const getLoser = async(name) => {
    await getPlayerFromName(name).then((data) => {
      setLoserId(`${data}`);
    })
  }
  ...
  useEffect(async() => {
    await findTournament(tournamentId, setTourney).then(async(data) => {
      ...
      setTourney(data);
      await getWinner(data[0].winner)
      await getLoser(data[0].runnerUp)
      setLoading(false);
    }).catch((err) => {
      console.log(err)
                    ...
                    <table className="table table-hover table-dark">
                      <tbody>
                        ...
                        <tr>
                          <th>Winner</th>
                          <td>
                            <Link className='text-lightaqua' to={"/players/" + winnerId} style={{ textDecoration: "none" }}>
                        ...
                        <tr>
                          <th>Loser</th>
                          <td>
                            <Link className='text-lightaqua' to={"/players/" + loserId} style={{ textDecoration: "none" }}>
                              { tourney[0].runnerUp } ({ tourney[0].runnerUp_loc })
                            </Link>
                          </td>
                        </tr>
                        <tr>
